...
### Fixed
- **[ui] Components import the child elements their templates use.** `ag-config-panel` (ag-switch) and the fullscreen player (ag-volume-popover) relied on the app-wide registration in `main.js` and rendered without their toggles/volume control when mounted standalone (Storybook, isolated tests). A global audit of the same gap (~35 components) is tracked in the backlog.
