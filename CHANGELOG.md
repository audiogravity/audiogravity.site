...
### Fixed
- **[core] signal path — radio source shows "Radio" not the station name** — `origin_name` for radio is content metadata (station name, e.g. "Le Son Parisien"); it must not appear as a signal-path step. Canonical label "Radio" (and "Qobuz", "Tidal", "Library"…) now comes from the `_SIGNAL_PATH_SOURCE_LABELS` map keyed on `origin`. UPnP is the only exception where `origin_name` (the DLNA server name, e.g. "MinimServer") IS meaningful infrastructure metadata and is kept.
