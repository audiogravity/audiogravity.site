...
- **[ui] mini player — renderer badge in source row** — when a UPnP renderer is active and not bypassed, a `→ renderer_name` badge appears in the source row; the USB/TOSLINK connector badge is hidden in that state (already visible in the fullscreen signal path).
...
- **[ui] fullscreen player — NEXT / PREV routed to renderer** — when a renderer queue is active (`connected && !bypassed && queue_total != null`), the NEXT / PREV transport buttons call the new renderer endpoints instead of MPD. `can-next` / `can-prev` are derived from queue position so buttons are correctly disabled at boundaries.
...
- **[core] UPnP renderer — `RendererManager` multi-renderer pool** — replaces the singleton `UPnPRendererService`. Manages a `dict[udn → UPnPRendererService]`; exactly one renderer is active at a time (policy, not architecture). Config persisted in `upnp_renderer_known.json`; migrates silently from the legacy single-renderer format. Exposes `GET /upnp-renderer/known`, `get_active_service()`, `connect(renderer)`, `disconnect(udn)`, `discover()`. Auto-reconnect background task with exponential backoff (30 s → 5 min) shared with the previous singleton approach.
...
- **[core] player — native renderer transport fallback** — `_try_renderer_control()` dispatches toggle/next/prev/stop/seek to a non-local-MPD renderer (ARM UpMPd, Marantz, Linn…) when `get_now_playing()` returns nothing for that renderer. Wired in both `control()` paths: explicit `source_id` branch (source not found in pipeline) and auto-resolve branch (no active source). Closes the 503 on Play with a native renderer whose MPD is not in the local AG pipeline.
- **[core] GET /player/outputs** — unified output list: one entry per MPD `audio_output` config block (`type: "mpd_output"`, real device name from MPD `outputs` command) + all known UPnP renderers (`type: "upnp_renderer"`), with live `reachable` and `active` flags. Falls back to a single "Local DAC" entry when MPD is unreachable. Used by the UI output selector to build the complete output list in a single call.
...
- **[core] player — native renderer state when local pipeline is idle** — `_build_native_renderer_state()` provides title, artist, album, cover art, transport state, `output_label`, and `signal_path` when a non-local-MPD renderer is active but `get_now_playing()` returns nothing (the ARM renderer's MPD is on a different host). Fixes "Nothing playing" and "No output selected" with a native ARM renderer.
- **[core] RendererStatus — `cover_token` and `can_seek` fields** — `cover_token` exposes the current track's cover art token (from the active queue entry) so the fullscreen player can render cover art for a native renderer; `can_seek` exposes AVTransport seek capability for the playback controls.
...
- **[core] radio → renderer routing** — `RadioService.play()` now routes to the active UPnP renderer via `play_queue()` (AVTransport) when one is connected. Falls back to local MPD only when no renderer is active. The `RendererManager` is injected at startup via late binding (same pattern as `NowPlayingService.set_radio_lookup`). Error from the renderer propagates — no silent MPD fallback when the renderer is active.
...
- **[ui] UPnP renderer card — reworked as multi-output selector** — `ag-upnp-renderer-card` is now the unified output selector, loading from `GET /player/outputs`: one row per physical MPD audio output (USB, TOSLINK…) then all known UPnP renderers. Clicking a physical output calls `PUT /player/mpd-output/{output_id}` (enables exclusively, auto-disconnects renderer); clicking a renderer calls `PUT /{udn}/connection`. Active renderer shows Disconnect button (switches to first MPD output) + volume slider. States: Active (green) / Reconnecting (orange) / Idle (grey). Scan button at bottom filters already-known renderers. SSE sync updates `active`/`reachable` in the list without full reload; full reload triggered on SSE reconnection.
...
### Fixed
- **[ui] UPnP renderer card — double "Active" badge via SSE path** — `_onStatusEvent` did not clear `active` on the previously active renderer when a new one received `connected: true`. `_load()` (full reload) was correct; the SSE fast-path was not. Fixed: when `isNowActive=true`, all non-target renderers in `_known` now force `active: false`.
- **[core] UPnP renderer — `disconnect()` removed renderer from known list** — `disconnect()` filtered the renderer out of `_services` pool AND from `_known`, making it disappear from the card after disconnect and requiring a new Scan to re-add it. Fixed: only `_active_udn` is cleared; the renderer stays in `_known` so it can be re-selected without re-scanning.
...
- **[core] player — 503 on Play with native (ARM) renderer** — when the fullscreen player sent `source_id` explicitly (`_targetSourceId`), `control()` took the `if source_id:` branch, found nothing in `get_now_playing()` (native renderer MPD not in local pipeline), and returned 503 without reaching the renderer fallback. Fixed: `_try_renderer_control()` fallback now wired in both branches of `control()` — `if source_id:` (source not found) and `else` (no active source).
- **[core] UPnP renderer — `_publish_status(force=True)` didn't update the hash baseline** — on forced publishes (heartbeat, connect, disconnect), `_last_sse_hash` was not updated because the hash computation lived inside the `if not force:` block. A non-forced tick arriving immediately after had the wrong baseline, potentially either silently deduplicating a real change or emitting a duplicate. Hash is now always computed and `_last_sse_hash` always updated; only the early-return dedup check is gated on `not force`.
- **[core] UPnP renderer — `_resubscribe_and_refresh()` didn't restore reachability after SID recovery** — `_handle_update_result(success=True)` was not called after a successful `async_update()` post-resubscribe; `_reachable` stayed `False` and `_consecutive_failures` kept growing, so the renderer card showed "offline" for up to 30 s after the SID mismatch was resolved. Both success and failure paths now call `_handle_update_result()` symmetrically.
- **[core] UPnP renderer — `_retreat_queue` pre-decrement unguarded against `stop()` race** — `stop()` can clear `_queue` and reset `_queue_idx = 0` between the public `retreat_queue()` guard check and the unconditional `_queue_idx -= 1` inside `_retreat_queue()`. This left `_queue_idx = -1` permanently, making `_queue[-1]` silently index the last element on the next call. Guard added before the decrement (symmetric with `_advance_queue`'s overflow check).
- **[core] player — renderer included in signal path during startup reconnect window** — between `load_config()` and `connect()` completing, `get_status()` returns `connected=True, reachable=False`. `_build_state()` used only the `connected` flag, so it emitted a signal path with the renderer step during this window even though the DMR was not yet initialised. Guard tightened to require `connected AND reachable`.
- **[core] library — `asyncio.TimeoutError` not caught in MPD queue-list endpoint** — completion of the earlier album-endpoint fix: the second catch block (for `GET /library/queue`, MPD source) was still missing `asyncio.TimeoutError`, returning HTTP 500 on a stalled MPD instead of an empty list.
- **[ui] fullscreen player — "Up next" strip showed stale track after renderer disconnect** — `_nextTrack` was only cleared when the renderer sent `queue_next_title=null` while still connected. On disconnect or bypass the SSE payload has no `queue_total`, so neither branch executed and the strip kept displaying the last known next track. Cleared explicitly when `!connected || bypassed`.
- **[ui] fullscreen player — cover error token not reset between consecutive album tracks** — `_coverErrorToken` was only cleared on page reload or full source reconnect. Tracks within the same album can share the same `cover_token`; after one 404, the token was permanently suppressed for all subsequent tracks with that token. Token is now reset on every track-title or source change.
- **[ui] fullscreen player — idle renderer badge missing when nothing is playing** — the source row was conditioned on `origin || source_name || hasSignal`, all of which are falsy when the renderer is connected but idle (backend `active=None` → empty `signal_path`, no `origin`). Condition extended to include `_rendererActive`; a compact `→ renderer_name` badge is shown when `!hasSignal && _rendererActive`.
- **[ui] mini player — connector badge hidden for upmpdcli (local-MPD renderer)** — the badge was unconditionally hidden when any renderer was active. upmpdcli (`uses_local_mpd=true`) routes audio through the local MPD stack, so the physical USB/TOSLINK connector IS in the chain. Visibility now also passes when `_rendererStatus?.uses_local_mpd` is true.
- **[core] signal path — radio source shows "Radio" not the station name** — `origin_name` for radio is content metadata (station name, e.g. "Le Son Parisien"); it must not appear as a signal-path step. Canonical label "Radio" (and "Qobuz", "Tidal", "Library"…) now comes from the `ORIGIN_LABELS` map keyed on `origin`. UPnP is the only exception where `origin_name` (the DLNA server name, e.g. "MinimServer") IS meaningful infrastructure metadata and is kept.
- **[core] UPnP renderer — `_retreat_queue` rollback unsafe when `stop()` fires mid-retreat** — mirror of the earlier `_advance_queue` fix: `stop()` can clear the queue and reset `_queue_idx = 0` while `_play_queue_entry()` is awaiting; if playback then raises, the rollback `_queue_idx += 1` incremented 0 to 1 on an empty list. Guard `if self._queue:` added, symmetric with `_advance_queue`.
- **[core] UPnP renderer — `_uses_local_mpd` stale after disconnect or failed reconnect** — `disconnect()` now resets `_uses_local_mpd = True`; `get_status()` re-derives it from the saved location URL when `_dmr is None` (startup reconnect window) via the extracted `_is_local_renderer()` helper, preventing the wrong signal-path from appearing.
- **[core] library — `asyncio.TimeoutError` not caught in album/queue list endpoints** — a stalled MPD during restart raised `asyncio.TimeoutError` which bypassed the `(OSError, ConnectionRefusedError, BrokenPipeError)` empty-list handler and returned HTTP 500.
- **[core] player — renderer status error swallowed silently in `_build_state()`** — bare `except Exception: pass` replaced with `logger.debug(..., exc_info=True)` so import errors, container misconfiguration, or schema regressions leave a trace.
- **[core] Roon — suppress ERROR logs when roonbridge service is stopped** — when the `roonbridge` systemd service is inactive, `connect()` now gates on `ActiveState` via the existing D-Bus client before creating `RoonApi`; the `roonapisocket` daemon threads that emit `ERROR: Connection is not (yet) ready!` are never started. Falls back silently when D-Bus is unavailable (ARM / non-systemd setups).
- **[core] Roon — TCP pre-check before mDNS discovery** — `_sync_connect()` now verifies the configured host is reachable (port 9330) before running `RoonDiscovery.first()`, which can block for several seconds when Roon Core is offline. Re-checks TCP if discovery resolves a different address.
- **[core] Qobuz proxy — Range headers relayed end-to-end** — `GET /qobuz/stream/{track_id}` now forwards the `Range` request header to the CDN and relays `Content-Range`, `Content-Length` and `Accept-Ranges` response headers with the correct status code (206 Partial Content). UPnP renderers can now seek within a Qobuz track without restarting the stream.
- **[core] UPnP renderer — `_on_dmr_event` called `_check_queue_advance` in bypass mode** — when bypass was active, NOTIFY events still triggered queue-advance detection; only the poll-loop heartbeat suppressed it. Now guarded at the event source with the same `if not self._bypassed` check.
- **[core] UPnP renderer — `_check_queue_advance` stacked `create_task` on rapid NOTIFYs** — end-of-track NOTIFY can fire several times in quick succession; each call created a new coroutine object. Fixed by storing the task reference in `_advance_task` and skipping if the previous task is still running (same deduplication pattern as `_publish_task`).
- **[core] UPnP renderer — `_ext_stream_key` missed `/qobuz/stream/` proxy URLs** — Qobuz proxy URLs (path `/qobuz/stream/{id}`) were not classified as `qobuz:` keys; they fell through to the raw URL fallback, causing a spurious AVTransport art fetch and potentially incorrect cover art. Added a path-anchored check before the CDN `?eid=` query-param check.
- **[core] UPnP renderer — `_advance_queue` rollback unsafe when `stop()` fires mid-advance** — `stop()` can clear the queue and reset `_queue_idx = 0` while `_play_queue_entry()` is awaiting; if the play then raises, the `except` block decremented 0 to -1. Fixed: rollback only applies when the queue is still populated.
- **[core] UPnP renderer — `_publish_status(force=True)` computed JSON hash unnecessarily** — `json.dumps(sort_keys=True)` + MD5 ran on every force-publish (heartbeat, connect, disconnect) even though the dedup guard is bypassed on `force=True`. Hash computation now skipped in the force path.
- **[core] UPnP renderer — renderer pinged every 30 s in SUBSCRIBE mode** — `async_update(do_ping=True)` (HTTP GET to the device description URL) fired on every SSE heartbeat. Decoupled from the heartbeat: ping interval extended to 60 s while the SSE heartbeat remains 30 s, halving outbound HTTP GETs to the renderer.
- **[core] Qobuz — `ClientSession` created per API call** — `get_stream_url()`, `handle_callback()`, and `validate_token()` each opened a new `aiohttp.ClientSession` (fresh TCP + TLS handshake per call). Migrated to `get_shared_session()` from `core.http` (shared pool, identical to the Tidal proxy and cover-art lookup paths).
- **[core] UPnP renderer — NOTIFY SID mismatch after AG restart** — after a backend restart the new `UpnpEventHandler` had an empty SID table; the renderer continued sending NOTIFYs with the old SID, which were silently dropped (412 Precondition Failed returned to renderer, not logged). The DMR cache stayed stale for up to 30 s (next heartbeat). Fix: `handle_notify()` now detects 412, logs a warning with the SID, and schedules `_resubscribe_and_refresh()` — a background task that unsubscribes, re-subscribes (fresh SID), and calls `async_update()` immediately so the badge and transport state are accurate without waiting for the heartbeat.
- **[core] Qobuz — single-track play to renderer used expiring CDN URL** — `qobuz_queue()` for a single track was passing the raw HMAC-signed CDN URL (TTL ~30 min) to `renderer.play()`; after expiry the renderer re-fetched and got 403, stopping playback. Fixed: now uses the stable proxy URL like the multi-track album path.
- **[core] Qobuz proxy — CDN stall leaked aiohttp session indefinitely** — `ClientTimeout(total=None)` meant a stalled CDN connection held the session open forever. Fixed: `sock_read=300` (5 min idle → error, session closed).
- **[core] Qobuz proxy — network errors returned HTTP 500 instead of 503** — `except RuntimeError` in the proxy endpoint did not catch `aiohttp.ClientConnectorError` / `asyncio.TimeoutError`; those propagated as raw 500. Fixed: `except Exception`.
- **[core] UPnP renderer — NEXT / PREV returned ok:True when transition already in progress** — if `_queue_advancing` was set, `advance_queue()` / `retreat_queue()` silently no-oped and the router responded `{ok: True}`. Fixed: public methods raise `RuntimeError("Queue transition in progress")` → router returns 409.
