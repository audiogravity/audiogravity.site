...
- **[core] GET /player/outputs** — unified output list: local MPD ("Local DAC", always present) + all known UPnP renderers with live `reachable` and `active` flags. Used by the UI output selector to build the complete output list in a single call.
...
- **[ui] UPnP renderer card — reworked as multi-output selector** — `ag-upnp-renderer-card` is now the output selector: Local DAC always first (click → disconnect active renderer), known renderers listed below (click → `PUT /{udn}/connection`). Active renderer shows Disconnect button + volume slider. States: Active (green) / Reconnecting (orange) / Idle (grey). Scan button at bottom filters already-known renderers. SSE sync updates `active`/`reachable` in the list without full reload; full reload triggered on SSE reconnection.
