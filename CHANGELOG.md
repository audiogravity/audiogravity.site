...
- **Audio-stack per-service provisioning & guided configuration** (core + ui + ops): generate and manage minimal, bit-perfect configs for the audio services (mpd, upmpdcli, shairport), each wired to its own output.
  ...
  - **Core**: audio-output re-resolution (ALSA card indices can shift after a USB re-enumeration) now runs at boot from a dedicated `ag-audio-reresolve` systemd oneshot ordered **before** mpd/shairport (core binary `--reresolve-audio-outputs`, no service restart) so the services open the correct device on the first play; replaces the previous core-startup task.
  ...
  - **ops**: `build-core-package.sh` installs/removes the `ag-audio-reresolve.service` oneshot (no new sudoers — reuses the existing `audiogravity` NOPASSWD commands).
